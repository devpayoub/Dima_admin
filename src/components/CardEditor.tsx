...
export function CardEditor({ initialTemplate, ownerId, onSave }: CardEditorProps) {
  ...
  const handleAssetUpload = async (kind: CampaignAssetKind, file: File | null) => {
    if (!file) return;
    if (!ownerId) {
      throw new Error('No active owner account found for upload.');
    }
                    ...
                    <AccordionContent className="space-y-4 pt-2">
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between gap-3">
                                <Label htmlFor="totalStamps">{template.mode === 'points' ? 'Number of Steps' : 'Number of Stamps'}</Label>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                                <button
                                    type="button"
                                    onClick={() => handleChange('mode', 'stamps')}
                                    className={cn(
                                        "flex-1 py-2 px-3 rounded-lg border text-sm font-medium transition-colors",
                                        template.mode !== 'points' ? "bg-primary text-primary-foreground" : "bg-background hover:bg-muted"
                                    )}
                                >
                                    Stamps
                                </button>
                                <button
                                    type="button"
                                    onClick={() => handleChange('mode', 'points')}
                                    className={cn(
                                        "flex-1 py-2 px-3 rounded-lg border text-sm font-medium transition-colors",
                                        template.mode === 'points' ? "bg-primary text-primary-foreground" : "bg-background hover:bg-muted"
                                    )}
                ...
                <AccordionItem value="design">
                    ...
                    <AccordionContent className="space-y-6 pt-2">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-3">
                                <Label className="text-xs uppercase tracking-wider text-muted-foreground font-bold">{template.mode === 'points' ? 'Empty Step' : 'Empty Slot'}</Label>
                            ...
                            <div className="space-y-3">
                                <Label className="text-xs uppercase tracking-wider text-muted-foreground font-bold">{template.mode === 'points' ? 'Filled Step' : 'Filled Slot'}</Label>
